...
/*
 * Modified by Pavel Kirienko (pavel.kirienko@gmail.com)
 * Fixed some issues
 */
...
void CAN_ITConfig(CAN_TypeDef* CANx, uint32_t CAN_IT, FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_CAN_ALL_PERIPH(CANx));
  //assert_param(IS_CAN_IT(CAN_IT));          // Masks should be allowed here, so this assertion is useless. (pavel.kirienko@gmail.com)
  assert_param(IS_FUNCTIONAL_STATE(NewState));
